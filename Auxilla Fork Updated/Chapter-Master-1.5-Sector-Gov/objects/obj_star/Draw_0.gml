...
if (global.load == -1 && (obj_controller.zoomed || in_camera_view(star_box_shape()))) {
    ...
    // Ork clan icon (§16g): a small clan SYMBOL on the green label so you can tell at a glance which clan
    // runs this WAAAGH. The base colour stays green (Ork identity) — this is only an accent marker.
    if (owner == eFACTION.ORK) {
        ...
        if (is_struct(_olead)) {
            ...
            var _py = y + (12 * scale);
            var _pr = 6 * scale;
            ork_warband_draw_icon(_olead, _px, _py, _pr);   // this clan's OWN colour + icon (§16m)
